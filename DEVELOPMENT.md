...
| `provider` | TEXT | NOT NULL, DEFAULT 'openai' | AI 平台标识。取值：`openai`/`openai-stream`/`openai-compatible`、`google`/`gemini`、`anthropic`/`claude`、`openrouter`、`pollinations`、`exacg`、`microsoft-tts`。同一平台的别名等价（如 `google` 与 `gemini` 行为一致）。决定使用哪个 Vercel AI SDK provider 创建函数 |
...
type Provider = 'openai' | 'openai-stream' | 'openai-compatible' | 'google' | 'gemini' | 'anthropic' | 'claude' | 'openrouter' | 'pollinations' | 'exacg' | 'microsoft-tts';
...
const CreateChannelSchema = z.object({
    ...
    provider: z.enum(['openai', 'openai-stream', 'openai-compatible', 'google', 'gemini', 'anthropic', 'claude', 'openrouter', 'pollinations', 'exacg', 'microsoft-tts']).default('openai'),
 ...
 * 展开逻辑：每个模型会展开为多个“模型标识”返回：
 * 1) 真实 code（必选）
 * 2) aliases 中的每个别名（可选）
 * 去重逻辑：按“最终模型标识字符串”全局去重（同名仅保留一条）
 * 边界规则：
 * - 空字符串/全空白别名忽略
 * - alias === code 不重复返回
 *
...
说明：
- `data[].id` 可能是模型真实 `code`，也可能是该模型的 `alias`。
- 一个模型若配置了 `n` 个有效别名，则最多返回 `n + 1` 个模型项（含真实 `code`）。
- 返回列表按最终模型标识去重，不允许同名 `id` 重复。
...
1. 一级列表：按“模型标识”聚合，模型标识集合 = `model.code ∪ model.aliases`，显示“模型标识（可复制）”与聚合统计（渠道数、平均成功率、平均耗时）。
2. 二级列表：展开后显示同模型标识命中的全部渠道模型，列字段固定为：
   - 模型名
   - 模型别名（标签化）
   - 模型状态（标签化，中文）
   - 渠道名
...
3. 交互规则：
   ...
   - 一级分组若来源为 alias（非真实 code），该分组仅用于展示与筛选，不提供组级编辑入口；二级行编辑/删除保持可用。
   - 删除模型：规则同编辑（支持同名全量删除与单条删除）。
   - 排序规则固定（不提供排序选择控件）：
     - 一级模型分组固定按“模型标识字符串”升序（A-Z）。
     - 二级渠道行固定按 `channel_name` 升序（A-Z）。
     - `state.modelListControls` 仅保留筛选字段：`keyword`、`provider`、`status`、`callTypes`（数组，多选，默认全不选）。
   - 筛选操作栏样式规范：模型管理页筛选操作栏（间距、栅格、输入框高度、按钮尺寸、视觉层次）必须与调用日志搜索栏保持一致，仅允许文案与字段项不同。
   - `callType` 筛选交互规范：
     - 操作栏第一行仅保留关键词、Provider、状态、重置筛选。
     - 操作栏第二行平铺展示全部可选 `callType`，支持多选，无额外说明文案。
     - 默认行为为“全不选”；当 `callTypes` 为空数组时，筛选效果等价于“全选全部可见 `callType`”（即不过滤 `callType`）。

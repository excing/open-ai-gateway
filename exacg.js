...
class ExacgImageModelV3 {
  ...
  async doGenerate(options) {
    ...
    if (!response.ok) {
      throw new Error(`Exacg upstream returned ${response.status}: ${errorMessage}`);
    }
    if (errorMessage) {
    ...
    const resource = await downloadFile(json.data.image_url);
    if (!resource) {
      throw new Error(`Exacg upstream error: Failed to convert image format`);
    }
